//
//  NSAttributedString+MyAttributedStringAdditions.m
//  ASAP
//
...
@implementation NSAttributedString (MyAttributedStringConstructors)
...
@end
@implementation NSMutableAttributedString (MyAttributedStringStyleModifiers)
...
-(void)setTextBold:(BOOL)isBold range:(NSRange)range 
{
	...
	do {
		...
		if (newFont) 
        {
			...
			NSLog(@"AttributedLabel Warning: can't find a bold font variant for font %@. Try another font family (like Helvetica) instead.",fontName);
		}
